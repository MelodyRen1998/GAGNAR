...
### EM algorithm
EM.NAR<-function(Ymat, W, Z, K, seed)
{
  if (is.null(Z))
...
Cluster.NAR<-function(Ymat, W,  Z, K, method = "complete", plot = F, group = NULL, seed = F)
{
  ...
  zhat <- yy_dat$group %>% as.factor()
  zzhat <- model.matrix(~zhat-1)
  XX = yy_dat[,2:(ncol(yy_dat)-2)]
  yy_dat$Yhat = diag(as.matrix(XX) %*% t(zzhat %*% as.matrix(theta_est[,-1])))

  sigma_est = ddply(yy_dat, .(group), function(x){
    sqrt(sum((x$Y - x$Yhat)^2)/nrow(x))
  })
  return(list(theta = t(theta_est)[-1,], sigma = t(sigma_est)[-1,], alpha = alpha, group = memb))
}
...
### This function is used to estimate the NAR model
betaOLS_timeZ<-function(Ymat, W, Z)                                                                                  ### OLS estimation for theta: eq (2.8)
{
  Ymat1 = W%*%Ymat                                                                                             ### obtain WY
  Time = ncol(Ymat)-1                                                                                          ### Time doesn't count Y0
  if (is.null(Z))
...
### EM algorithm
EM.NAR_Z<-function(Ymat, W, Z, K, seed = F)
{
  if (is.null(Z))
    npara = 3   # number of parameters
  else
  ...
  ### set initial values for the algorithm
  nar_para = betaOLS_timeZ(Ymat, W, Z)  # generate the initial estimator by the NAR model
  if (K==1)
    return(nar_para)
  theta = t(sapply(nar_para$theta, function(x) runif(K, min = x-0.05, max = x+0.05))) # generate the initial values by small perturbations of NAR estimator
  sigma = rep(nar_para$sigmaHat, K) # for simplicity set sigma the same for initial values
...
Cluster.NAR_TimeZ <-function(Ymat, W, Z, K, method = "complete", plot = F, group = NULL, seed = F)
{
  N = nrow(Ymat)
  Time = ncol(Ymat)
  Ymat1 = W%*%Ymat     
  ...
  if (is.null(Z))
    ...
    yy_dat = data.frame(id = rep(1:N, each = Time - 1),
                        ...
                        net = as.vector(t(Ymat1[,-ncol(Ymat)])),
                        lagY = as.vector(t(Ymat[,-ncol(Ymat)])),
                        Z[setdiff(1:(N*Time), (1:n.nodes)*Time), ],
                        Y = as.vector(t(Ymat[,-1])))
