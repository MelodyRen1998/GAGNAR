...
GWCRP <- function(Y_tilde, X_tilde, dmat, alpha, h, niterations, 
                  a0, b0, tau0, sigma0, initNClusters, verbose)
{
  ## Model: z|(alpha, h) \sim gwCRP(alpha, h)
  ...
  # sigma2 <- c(sigma1_true, sigma2_true, sigma3_true)
  # theta <- matrix(c(theta1, theta2, theta3), byrow = T, nrow = initNClusters)
  ...
  # sigma2 <- c(sigma1_true, sigma2_true, sigma3_true)
  # sgm_true <- c(sigma1_true, sigma2_true, sigma3_true)
  # given sigma2_k in cluster_k, we get theta_k from MVN(tau_0, sigma2_k * sigma0)
  # theta <- sapply(sigma2, function(x) {rmvnorm(1, mean = tau0, sigma = (x * sigma0), checkSymmetry = F)}) %>% t() # dim(theta) = [initNClusters, (p+3)]
  theta <- tryCatch({
  ...
  for (iter in 1:niterations)
  {
    ...
    { # determine whether ith component is a singleton
      ...
      if (clustersize_of_iclu > 1){
        ...
        # ==========================================================================
        # ==== ??? add constant to the log scale density to obtain positive ones ??? ====
        # clusterProbs_log <- clusterProbs_log - max(clusterProbs_log)
        # clusterProbs <- exp(clusterProbs_log)
        clusterProbs <- clusterProbs_log
        # cat("sample: ",i, "ClusterProbs: ", clusterProbs, "\n")
        # ==========================================================================
        
        # choose the cluster number for ith observation
        # cat("sample from ", nClusters+1, " cluster with prob: ", clusterProbs, "\n")
        ...
        if (cluster.i > nClusters) {
          ...
          # new_theta <- rmvnorm(1, mean = tau0, sigma = (new_sigma2 * sigma0))
          new_theta <- tryCatch({
      ...
      if (clustersize_of_iclu <= 1) {
        ...
        # cat(abs(det(sigma_star)), abs(det(sigma0)), "\n")  ## TODO: print determinant ???????
        # cat("log cluster prob is: ", clusterProbs_log, "\n")
        # ==========================================================================
        # ==== ??? add constant to the log scale density to obtain positive ones ??? ====
        # clusterProbs_log <- clusterProbs_log - max(clusterProbs_log)
        # clusterProbs <- exp(clusterProbs_log)
        ...
        # choose the cluster number for ith observation
        # cat("sample from ", nClusters, " cluster with prob: ", clusterProbs, "\n")
        cluster.i <- sample(1:nClusters, size = 1, prob = clusterProbs)
        clusterAssign[i] <- cluster.i
        
        ## check 还是需要加一个new cluster的 没有问题这里
        if (cluster.i == nClusters) {
          new_sigma2 <- rinvgamma(1, shape = a0, scale = b0)
      ...
      # cat("finish loop for the ", i, " the sample \n")
      # cat("dim of theta:", dim(theta), "\n")
      # cat("number of cluster after loop for the", i, "th sample: \n")
      # print(clusterSizes)
    }
    ...
    # cat("total number of clusters: ", nClusters, "\n")
    for (r in 1:nClusters){
      # cat("cluster: ", r, "\n")
      
      ## sigma*
      X_group_r <- X_tilde[ 2:t, which(clusterAssign == r), ]
      # cat("dimension of X_group_r: ", dim(X_group_r)[1], dim(X_group_r)[2], dim(X_group_r)[3], "\n")

      if (sum(clusterAssign == r) == 1) {  # 如果只有一个node，那X_group_r是一个矩阵
        X_tilde_t_sum <- t(X_group_r) %*% X_group_r
      } else if (sum(clusterAssign == r) > 1) {  # 如果有多个nodes，就按照列表逐一计算加和
      ...
      # post_sigma_star <- ginv((t-1) * sigma0_inv + X_tilde_t_sum)
      post_sigma_star <- ginv(sigma0_inv + X_tilde_t_sum)
      
      ## tau*
      # Y_tilde_array <- array(Y_tilde, dim = c(dim(Y_tilde), 1))  # dim(Y_tilde_array) = [T,N,1]
      Y_group_r <- Y_tilde[ 2:t, clusterAssign == r]  # dim(Y_group_r) = [T, Nr]
      
      if (sum(clusterAssign == r) == 1) {  # 如果只有一个node，那X_group_r是一个矩阵, Y_group_r是一个vector
        X_tilde_t_Y_sum <- t(X_group_r) %*% Y_group_r  # dim = [d, 1]
      } else if (sum(clusterAssign == r) > 1) {  # 如果有多个nodes，就按照列表中的nodes逐一计算加和
        X_tilde_t_Y <- lapply(1:dim(Y_group_r)[2], function(x) {t(X_list_bygroup[[x]]) %*% Y_group_r[,x]})
        X_tilde_t_Y_sum <- Reduce('+', X_tilde_t_Y)
      }
      
      # post_tau_star <- post_sigma_star %*% (((t-1)*sigma0_inv) %*% tau0 + X_tilde_t_Y_sum)
      post_tau_star <- post_sigma_star %*% (sigma0_inv %*% tau0 + X_tilde_t_Y_sum)
      
      ## a*
      # post_a_star <- a0 + (sum(clusterAssign == r) + t - 1)/2
      # post_a_star <- a0 + (sum(clusterAssign == r))/2
      # post_a_star <- (t-1) * (a0 + (sum(clusterAssign == r) + d)/2 + 1) - d/2 - 1
      post_a_star <- a0 + (t-1) * sum(clusterAssign == r)/2
      
      ## b*
      # N_k <- sum(clusterAssign == r)  # 试一下把sum_i放在中括号外面
      Y_tilde_sum <- sum(apply(as.matrix(Y_tilde[2:t, clusterAssign == r]), 2, function(x){t(x) %*% x}))
      # post_b_star <- ((t-1) * b0 + (t(tau0) %*% ((t-1)*sigma0_inv) %*% tau0 + Y_tilde_sum - t(post_tau_star) %*% (((t-1)*sigma0_inv) + X_tilde_t_sum) %*% post_tau_star)/2) %>% as.numeric()
      post_b_star <- (b0 + (t(tau0) %*% (sigma0_inv) %*% tau0 + Y_tilde_sum - t(post_tau_star) %*% (sigma0_inv + X_tilde_t_sum) %*% post_tau_star)/2) %>% as.numeric()
      
      # cat("post a* and b* are:", post_a_star, post_b_star, "\r")   # ?????
      ### check: print sigma2 and theta
