...
Cluster.NAR<-function(Ymat, W,  Z, K, method = "complete", plot = F, group = NULL, seed = F)
{
  ...
  #   invXX = ginv(crossprod(X))                                                                                   ### the response vector
  #   thetaEst = invXX%*%colSums(X*x$Y) 
  #   sig2Est = as.numeric(t(Y) %*% (diag(1, nrow(X)) - X%*%invXX%*%t(X)) %*% Y) / nrow(X)
  #   covEst = sig2Est * invXX
  #   df = data.frame(matrix(c(thetaEst), nrow = 1))
  # })
  
  if (is.null(Z)) {p = 0}
  theta_est = matrix(0, nrow = p+3, ncol = K)
  sig2Est = numeric(K)
  covEst = list()
  for (g in 1:max(memb)) {
  ...
  return(list(theta = t(theta_est), alpha = alpha, group = memb, sig2 = sig2Est, cov = covEst))
}
