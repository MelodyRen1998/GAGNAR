...
pdf(width=4, height=4, file = "./report/city_des_2005_2016_GDP.pdf")
ggplot(yy_dat, aes(x = value)) +
  geom_histogram(binwidth = 0.01, color = "black", fill = "grey") +
  theme_bw() +
  labs(x = "Average GDP Growth Rate", y = "Frequency") +
...
tryCatch({
  while(flag) {
    ...
    save(real_res, file = "./report/city_res_GDP.rda")
    ...
    # (2) est
    K = 6
    GNAR_summary = Cluster.NAR(Ymat_train, W, Zmat, K, method="kmeans", seed = n_test)
    ...
    # }
    save(GNAR_summary, file = "./report/city_res_gnar_GDP.rda")
    # (3) predict
...
# write.csv(pred_rmse[,1:(n_test - 1)], "./report/pred_rmse_city.csv", row.names = F)
