...
yy_dat2 <- data.frame(YY = 1:ncol(Ymat), value = colMeans(Ymat))
pdf(width=4, height=4, file = "./report/stk_line_2020.pdf")
ggplot(yy_dat2, aes(x = YY, y = value, group = 1)) +
  geom_line() +
  theme_bw() +
  labs(x = "Week", y = "Average Return Rate") +
  theme(axis.text = element_text(size = 10)) +
  ylim(-0.2,0.2)
dev.off()
...
while (flag) {
  ...
  # if((train_start == 1) && (train_window == 40) && (h %in% c(0,2))) {
  ...
  if(train_start == 1) {
    save(res_topstk, file = "./report/stock_res_supp_full_train40_test6_alpha_1.rda")
    break
    }
  besth_ind <- which.max(unlist(lapply(res_topstk, function(x){x$LPML})))
  # besth_ind = 2  # 第二个（用于固定一个h预测）
  ...
  # # save gagnar estimation
  gagnar_res = list(besth, ZZ_best, Theta, Sigma2)
  if (train_start == 1) {
    save(gagnar_res, file = "./res/est_res_gagnar_alpha_0_1_sparse_3_train30_test10.rda")
  }
  ...
  # save crp estimation
  crp_res = list(ZZ_crp, Theta_crp, Sigma2_crp)
  if (train_start == 1) {
    save(crp_res, file = "./res/est_res_crp_alpha_0_1_sparse_3_train30_test10.rda")
  }
  ...
  # }
  gnar_res = list(Theta, Sigma2)
  if (train_start == 1) {
    save(gnar_res, file = "./res/est_res_gnar_alpha_0_1_sparse_3_train30_test10.rda")
  }
  ...
  if (train_start == 1) {
    save(nar_est, file = "./res/est_res_nar_alpha_0_1_sparse_3_train30_test10.rda")
  }
  ...
  if (train_start == 1) {
    save(fit, file = "./res/est_res_arma_alpha_0_1_sparse_3_train30_test10.rda")
  }
  ...
  if (train_start == 1) {
    save(fit, file = "./res/est_res_ar_alpha_0_1_sparse_3_train30_test10.rda")
  }
