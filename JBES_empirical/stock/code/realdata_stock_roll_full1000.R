...
# load data
# =========
load("./stock_data/rda_full_1000/Ymat_top.rda")
load("./stock_data/rda_full_1000/Zmat_top.rda")
load("./stock_data/rda_full_1000/Adjmat_top.rda")
cat("network density is", sum(adj.mt)/(nrow(adj.mt)^2 - nrow(adj.mt)), "\n")
load("./stock_data/rda_full_1000/dmat_top.rda")  # d.matrix
Zmat <- scale(Zmat)
...
while (flag) {
  ...
  if(train_start == 1) {save(res_topstk, file = "./report/stock_res_supp.rda")}
...
write.csv(pred_rmse[,1:(n_test - 1)], "./report/pred_rmse_stock_train40_test6_supp.csv", row.names = F)
